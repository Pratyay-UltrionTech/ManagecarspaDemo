...
function shortLabel(iso: string): string {
  // iso is either YYYY-MM-DD (daily) or YYYY-MM-DD weekly start
  ...
  return (
    <Card className="border-slate-200/60 shadow-sm">
      <CardContent className="flex flex-col gap-3 p-4">
        <div className={cn('flex size-9 shrink-0 items-center justify-center rounded-lg ring-1 ring-inset ring-black/5', bg)}>
          <Icon className={cn('size-4.5', accent)} strokeWidth={1.8} />
        </div>
        <div className="min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 leading-tight">{title}</p>
          <p className="mt-1 text-xl font-semibold tracking-tight text-slate-900 break-all">{value}</p>
          {sub && <p className="mt-0.5 text-[10px] text-slate-400 leading-snug">{sub}</p>}
        </div>
      </CardContent>
    </Card>
  );
...
export default function RevenueReportPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/reports/revenue-detail?${params}`, {
      ...
      const json = (await res.json()) as RevenueDetailData;
      setData(json);
      setHasLoaded(true);
    } catch (err: unknown) {
  ...
  }, [session?.accessToken, dateFrom, dateTo, channel, paymentMethod]);
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-lg text-xs">
        ...
        {payload[0]?.payload?.count != null && (
          <p className="mt-1.5 border-t border-slate-100 pt-1.5 text-slate-400">
            {payload[0].payload.count} booking{payload[0].payload.count !== 1 ? 's' : ''}
          </p>
        )}
      </div>
    );
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-6">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-slate-500">
          Analyse gross revenue, net revenue, discounts, tips and per-booking averages across any date range.
        </p>
      </div>
      ...
      <Card className="border-slate-200/60 shadow-sm">
        <CardHeader className="pb-3 pt-4 px-5">
          ...
          <CardDescription className="text-xs">Select the date range, channel, and payment method then click Run Report.</CardDescription>
        </CardHeader>
        <CardContent className="px-5 pb-5 pt-0">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            {/* Payment method */}
            <div className="space-y-1.5">
              <Label className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Payment Method</Label>
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            {hasLoaded && !loading && (
              <span className="text-[11px] text-slate-400">
                ...
                {data?.booking_count != null && ` · ${data.booking_count} booking${data.booking_count !== 1 ? 's' : ''}`}
              </span>
            )}
          </div>
        </CardContent>
        ...
        <>
          <div className="grid gap-4 grid-cols-2 lg:grid-cols-5">
            ...
            <KpiCard
              ...
              value={fmt(data.tips_collected)}
              sub={`Across ${data.booking_count} bookings`}
              icon={HandCoins}
            ...
            <KpiCard
              ...
              value={fmt(data.avg_per_booking)}
              sub="Net revenue ÷ bookings"
              icon={ReceiptText}
          ...
          <Card className="border-slate-200/60 shadow-sm">
            ...
            <CardContent className="p-5 pt-2">
              {data.chart.length === 0 || data.booking_count === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={320}>
                  ...
                  >
                    ...
                    <Legend
                      ...
                      iconSize={8}
                    />
                    <Bar dataKey="total_revenue" name="Total Revenue" fill="#6366f1" radius={[3, 3, 0, 0]} />
                    <Bar dataKey="net_revenue" name="Net Revenue" fill="#10b981" radius={[3, 3, 0, 0]} />
                    <Bar dataKey="discount" name="Discount" fill="#f43f5e" radius={[3, 3, 0, 0]} />
                    <Bar dataKey="tips" name="Tips" fill="#f59e0b" radius={[3, 3, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </CardContent>
      ...
      {!data && !loading && (
        <div className="flex h-64 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/40">
          <div className="text-center">
            ...
            <p className="mt-1 text-xs text-slate-400">Revenue data will appear here</p>
          </div>
        </div>
      )}
    </div>
