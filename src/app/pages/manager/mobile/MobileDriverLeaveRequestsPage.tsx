...
import {
  mobileManagerListDriverLeaveRequests,
  mobileManagerUpdateDriverLeaveStatus,
  type MobileDriverLeaveRequest,
} from '../../../lib/mobileLeaveApi';
...
export default function MobileDriverLeaveRequestsPage() {
  const { session } = useMobileManagerSession();
  const [requests, setRequests] = useState<MobileDriverLeaveRequest[]>([]);
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    setError(null);
    try {
      const data = await mobileManagerListDriverLeaveRequests();
      setRequests(data);
    } catch (e) {
  ...
  const handleAction = async (id: string, status: 'approved' | 'rejected') => {
    ...
    try {
      const updated = await mobileManagerUpdateDriverLeaveStatus(id, status);
      ...
      {loading ? (
        ...
        <div className={cn(cardSurface, 'px-6 py-16 text-center')}>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Leave requests submitted by your drivers will appear here.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  {/* Left: driver info + leave details */}
                  <div className="min-w-0 flex-1 space-y-1.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-sm font-semibold text-foreground">
                        {req.driver_name || 'Driver'}
                      </p>
                      <span
                        className={cn(
